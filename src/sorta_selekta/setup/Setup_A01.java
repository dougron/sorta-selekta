...
public class Setup_A01 extends NodeManagerSetupObject{
	...
	public ArrayList<NodeListGenerator> makeNLGList(){
		...
		nlgList.add(makeKeysNLG(atList, cfList));
		nlgList.add(makeMelodyNLG(atList, cfList, cdList));
...
//		cio.controllerList.add(new VolumeInfo("vol", cio.trackIndex, 1.0, cio.trackType));		// 1.0 test value to see if this is proportionate to the level set on the mixer or an absolute level
	...
	private NodeListGenerator makeKeysNLG(ArrayList<AccentTemplate> atList, ArrayList<ChordForm> cfList){
		NodeListGenerator nlg = new NodeListGenerator("Keys", "Y");
		nlg.atList = atList;
		nlg.cfList = cfList;
		...
		nlg.makeNodeList();
		nlg.playInject = new ClipInjectorObject(4, 0, ClipInjectorObject.ofTrackType);
		nlg.cueInject = new ClipInjectorObject(10, 0, ClipInjectorObject.ofTrackType);
...
//		nlg.proList.add(new PlugInSlowWah(0, "vol"));		// volume controller plugins are a bad idea
	...
	private ArrayList<ChordForm> getCFList(){
		...
		cfList.add(new ChordForm(getA01CFClip()));
		
		return cfList;
	}
	private LiveClip getA01CFClip() {
		LiveClip lc = new LiveClip(0, 0);
		
		lc.addNote(50, 0.0, 16.0, 100, 0);		// Dm7
		lc.addNote(53, 0.0, 16.0, 100, 0);
		lc.addNote(57, 0.0, 16.0, 100, 0);
		lc.addNote(60, 0.0, 16.0, 100, 0);
		
		lc.addNote(43, 16.0, 8.0, 100, 0);		// G7
		lc.addNote(47, 16.0, 8.0, 100, 0);
		lc.addNote(50, 16.0, 8.0, 100, 0);
		lc.addNote(53, 16.0, 8.0, 100, 0);
		
		lc.addNote(50, 24.0, 4.0, 100, 0);		// Dm7
		lc.addNote(53, 24.0, 4.0, 100, 0);
		lc.addNote(57, 24.0, 4.0, 100, 0);
		lc.addNote(60, 24.0, 4.0, 100, 0);
		
		lc.addNote(48, 28.0, 4.0, 100, 0);		// C7
		lc.addNote(52, 28.0, 4.0, 100, 0);
		lc.addNote(55, 28.0, 4.0, 100, 0);
		lc.addNote(58, 28.0, 4.0, 100, 0);
		
		lc.loopEnd = 32.0;
		lc.length = 32.0;
		lc.name = "A01Chords";
		lc.signatureNumerator = 4;
		lc.signatureDenominator = 4;
